...
* Create a project S3 bucket using the AWS Cloudshell
    ...
    ```sh
    aws s3 mb s3://jannet-data-pipelines/
    ```
...
* Copy the data from the udacity bucket to the home cloudshell directory: 
    ...
    ```sh
    aws s3 cp ~/log-data/ s3://jannet-data-pipelines/log-data/ --recursive
    aws s3 cp ~/song-data/ s3://jannet-data-pipelines/song-data/ --recursive
    aws s3 cp ~/log_json_path.json  s3://jannet-data-pipelines/
    ```

* List the data in bucket to be sure it copied over: 

    ```sh
    aws s3 ls s3://jannet-data-pipelinese/log-data/
    aws s3 ls s3://jannet-data-pipelines/song-data/   
    ```

### Airflow DAGs 

#### `final_project` DAG overview
...
3. Run `finaly_project` DAG to trigger the ETL data pipeline
